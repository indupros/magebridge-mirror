...
class MageBridgeTemplateHelper
{
    ...
    {
        return self::isPage('catalog/product/*');
    }
     ...
     * @return bool
     */
    static public function isCheckoutPage()
    {
        if (self::isPage('checkout/*') || self::isPage('onestepcheckout/*')) {
            return true;
        }
        return false;
    }
    ...
    static public function load($type, $file = null)
    {
        ...
        switch($type) {
            case 'jquery':
                ...
                if (MagebridgeModelConfig::load('use_google_api') == 1) {
                    ...
                    //} else {
                        $document->addScript($prefix.'://ajax.googleapis.com/ajax/libs/jquery/1.6.4/jquery.min.js');
                    //}
                } else {
                    self::load('js', 'jquery/jquery-1.4.2.min.js');
                }
        ...
        if ($type == 'js') {
            ...
            if (stristr($path, 'prototype')) {
                ...
                $file = basename($path);
                $path = dirname($path);
                JHTML::_( 'script', $file, $path.'/', false);
            }
    ...
    {
        switch($variable) {
            case 'jquery': return 'jquery/jquery-1.6.4.min.js';
        }
    }
