...
class MageBridgeModelProxy
{
    ...
    public function getCURL($url, $type = 'get', $arguments = null, $run_bridge = false)
    {
        ...
        if ($run_bridge == true) {
            ...
            foreach ($cookies as $cookie_name) {
                ...
                curl_setopt($handle, CURLOPT_COOKIE, $cookie_name.'='.$cookie_value);
            }
        }
        ...
        if (!empty($matches)) {
            foreach ($matches[0] as $index => $match) {
                ...
                if (!headers_sent()) setcookie($sessionName, $sessionId, 0, '/', '.'.JURI::getInstance()->toString(array('host')));

                // Store this cookie also in the default Joomal! session (in case extra cookies are disabled)
                $session = JFactory::getSession();
                $session->set('magebridge.cookie.'.$sessionName, $sessionId);
            }
        ...
        if ($this->_allow_redirects && ($this->_head['http_code'] == 301 || $this->_head['http_code'] == 302 || !empty($matches))) {
            ...
            if ($location == $bridge->getJoomlaBridgeUrl() && $bridge->getHttpReferer() != '') {
                $location = $bridge->getHttpReferer();
            }
    ...
    public function getCookieFile()
    {
        ...
        return $conf->getValue('config.log_path').'/magento.tmp';
    }
}
