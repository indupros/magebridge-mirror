...
class plgSystemMageBridge extends JPlugin
{
    ...
    public function onAfterDispatch()
    {
        ...
        // Preload all the Joomla! modules
        //$this->preloadModules();
    ...
    private function redirectNonSef()
    {
        ...
        if ($enabled == 1 && empty($post) && $config->getValue('sef') == 1 && JRequest::getCmd('option') == 'com_magebridge') {
    ...
    private function handleJavaScript()
    {
        ...
        // Load the whitelist
        $whitelist = JFactory::getApplication()->get('magebridge.script.whitelist');
        ...
        // Load the blacklist
        $blacklist = JFactory::getApplication()->get('magebridge.script.blacklist');
    ...
    private function preloadModules()
    {
        ...
        if (empty($module_chrome) && empty($module_show_title)) return;
        ...
        foreach ($modules as $index => $module) {
            if (preg_match('/^mod_magebridge_/', $module->module)) {
                ...
                if (!empty($module_chrome)) $mod->style = $module_chrome;
                if (!empty($module_show_title)) $mod->show_title = (int)$module_show_title;
            }
        }
    }
