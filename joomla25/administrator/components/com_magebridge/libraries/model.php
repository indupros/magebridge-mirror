...
class YireoModel extends JModel
{
    ...
    public function __construct($tableAlias = null)
    {
        ...
        $this->_option = (!empty($classParts[0])) ? 'com_'.strtolower($classParts[0]) : JRequest::getCmd('option');
    ...
    {
        $classParts = explode('Model', get_class($this));
        $option = (!empty($classParts[0])) ? 'com_'.strtolower($classParts[0]) : JRequest::getCmd('option');
        return $option;
    }
}
