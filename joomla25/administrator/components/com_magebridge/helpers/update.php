...
class MageBridgeUpdateHelper
{
    ...
    public static function getLatestVersion($package)
    {
        ...
        if (empty($init)) {
            ...
            $arguments = array(
                'license' => MagebridgeModelConfig::load('license'),
                'domain' => $domain,
                'resource' => 'versions',
    ...
    public function downloadPackage($url, $target = false)
    {
        ...
        if (!$target) {
            $target = $config->getValue('config.tmp_path').'/'.JInstallerHelper::getFilenameFromURL($url);
        } else {
            $target = $config->getValue('config.tmp_path').'/'.basename($target);
        }
