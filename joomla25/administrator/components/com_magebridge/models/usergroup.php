 ...
 */
class MagebridgeModelUsergroup extends JModel
{
    /**
     * Item id
     *
    ...
    public function store($data)
    {
        ...
        if (empty($data['joomla_group'])) {
            $this->setError(JText::_('No Joomla! usergroup was selected'));
            return false;
        }

        if (empty($data['magento_group'])) {
            $this->setError(JText::_('No Magento customergroup was selected'));
            return false;
        }
    ...
    public function delete($cid = array())
    {
        ...
        if (count( $cid )) {
            ...
            $cids = implode( ',', $cid );
            $query = 'DELETE FROM #__magebridge_usergroups WHERE id IN ( '.$cids.' )';
    ...
    public function publish($cid = array(), $publish = 1)
    {
        ...
        if (count( $cid )) {
            ...
            $query = 'UPDATE #__magebridge_usergroups'
    ...
    private function _loadData()
    {
        ...
        if (empty($this->_data)) {
            $query = 'SELECT s.*' .
                ' FROM #__magebridge_usergroups AS s' .
    ...
    private function _initData()
    {
        ...
        if (empty($this->_data)) {
            ...
            $item->joomla_group = null;
            $item->magento_group = null;
            $item->ordering = 0;
            $item->published = 0;
            $item->checked_out = 0;
