...
class MageBridgeUrlHelper
{
    ...
    static public function getReplacementUrls()
    {
        ...
        if ($urls == null) {
            if (MagebridgeModelConfig::load('load_urls') == 1) {
                $query = "SELECT * FROM #__magebridge_urls WHERE published = 1 ORDER BY ordering";
                $db = JFactory::getDBO();
                $db->setQuery($query);
    ...
    static public function hasUrlSuffix()
    {
        ...
        if ($config->getValue('sef') == 1) {
            return (boolean)$config->getValue('sef_suffix');
        }
        return false;
    }
     ...
     * @return string
     */ 
    static public function route($request = null, $xhtml = true)
    {
