...
class MagebridgeModelDebug
{
    ...
    public static function addGeneral()
    {
        ...
        if ($flag == 0) {
            ...
            if (MagebridgeModelDebug::isDebug() == true) {
                MageBridgeModelDebug::getInstance()->notice( "License: ".MagebridgeModelConfig::load('license'));
            }
        }
    }
     ...
     * @return int
     */
    public function getDebugOrigin($value = null)
    {
    ...
    private function _writeLog($data = null)
    {
        if ($data) {
            ...
            $log_path = $conf->getValue('config.log_path');
            if (empty($log_path)) {
